...
import { CP7Data, pathsData, lineData } from './tagData';
import { styleText } from '../StyleText';
import { CPDrawer } from '../CPFunction';

export class CP7Drawer extends CPDrawer {
    private readonly topicGuideCP7GSelection: d3.Selection<SVGGElement, MouseEvent, HTMLElement, any>;
    ...
    public constructor(
        ...
        this.topicGuideCP7GSelection = svgSelection.append('g');
    ...
    public update() {
        ...
        // @ts-ignore
        if ((Array.isArray(highlightedGroup) && highlightedGroup.includes("g7")) ||
          highlightedGroup === "g7") {
        ...
        this.topicGuideCP7GSelection.selectAll('circle, path, ellipse, text, tspan, line').style(
            'opacity',
            function (d: any) {
                const name =
                ...
                if (Array.isArray(highlightedGroup)) {
                  //@ts-ignore
                  if (!highlightedGroup.includes("g7")) {
                      return 0.3;
                  }
        ...
        const lineGroups = this.topicGuideCP7GSelection
            .selectAll('g.CP7Line')
            .data(lineData)
            .enter()
            .append('g')
        ...
        lineGroups
            ...
            lineGroups.attr(
                "transform",
                "translate(-590, 17) scale(-0.85, 0.85) rotate(135)"
            );

        const groups = this.topicGuideCP7GSelection
            .selectAll('g') // 기존 요소도 선택
            .data(pathsData)
            //className st22: PHR, st17: 진행자, st21: LJS, st19: KJD, st20: JKT
            .join(
                (enter) => {
                    // 새로 추가되는 요소 처리
        ...
        const filteredPaths = groups.selectAll("path")
            .style("opacity", (d) => {
                ...
                //@ts-ignore
                if (Array.isArray(selectedBlock) && selectedBlock[1]?.length === 0 && Array.isArray(highlightedGroup) && highlightedGroup.includes("g7")) {
                    //@ts-ignore
                ...
                if (selectedBlock.length === 0) {
                    ...
                    if (highlightedClasses.length !== 0) {
                        ...
                        if (highlightedClasses.includes('CONS')) {
                            //@ts-ignore
                            return ['st22', 'st21'].includes(d.className) ? 1 : 0.3; // PHR, LJS
                        }
                        // (2) 개별 참가자가 강조된 경우
                        //@ts-ignore
                    ...
                    if (highlightedGroup === "g7" || !highlightedGroup) {
                        return 1;
                    }
                    return 0.3;
                }
        ...
        CP7Data.forEach((groupData, i) => {
            ...
            const group = this.topicGuideCP7GSelection
                .append('g')
                ...
                group.selectAll('*').style('opacity', () => {
                    if (Array.isArray(highlightedGroup)) {
                        //@ts-ignore
                        return highlightedGroup.includes("g7") ? 1 : 0.3;
                    }
                    return highlightedGroup === "g7" || !highlightedGroup ? 1 : 0.3;
                });
                    ...
                    group
                        ...
                        .style('opacity', () => {
                            ...
                            if (Array.isArray(selectedBlock) && selectedBlock.length > 1) {
                                //@ts-ignore
                                if(Array.isArray(selectedBlock[1]) && selectedBlock[1].length === 0 && highlightedGroup.includes("g7")){
                            ...
                            // 🔹 `highlightedGroup`이 배열이면 `includes("g7")`로 체크
                            if (Array.isArray(highlightedGroup)) {
                                //@ts-ignore
                                if (!highlightedGroup.includes("g7")) {
                                    return 0.3;
                            ...
                            // 🔹 `highlightedGroup`이 "g7"일 경우 처리
                            //@ts-ignore
                            if ((Array.isArray(highlightedGroup) && highlightedGroup.includes("g7")) || highlightedGroup === "g7") {
                                if (Array.isArray(selectedBlock) && selectedBlock.length > 1 && Array.isArray(selectedBlock[1])) {
                                    //@ts-ignore
                                    if (selectedBlock[1].length > 1 && 
                                        (selectedBlock[1][0] === element.onClick || selectedBlock[1][1] === element.onClick)) {
                ...
                if (element.type === 'ellipse') {
                    ...
                    group
                        ...
                        .style('opacity', () => {
                            ...
                            // 🔹 highlightedGroup이 배열일 경우 "g7"이 포함되어 있는지 확인
                            if (Array.isArray(highlightedGroup)) {
                                //@ts-ignore
                                if (!highlightedGroup.includes("g7")) {
                                    return 0.3;
                ...
                if (element.type === 'text') {
                    ...
                    text
                      ...
                      .style('opacity', () => {
                        if (highlightedGroup) {
                            //@ts-ignore
                            if(element.content[0].text !== "국방 예산 이슈 및" && element.content[0].text !== "토론 마무리"
                                //@ts-ignore
                                && element.content[0].text !== "모병제를 위한" && element.content[0].text !== "군 예산조정"
                                //@ts-ignore
                                && element.content[0].text !== "마무리발언"
                            ){
                                return 1;
                            }
                          if (Array.isArray(highlightedGroup)) {
