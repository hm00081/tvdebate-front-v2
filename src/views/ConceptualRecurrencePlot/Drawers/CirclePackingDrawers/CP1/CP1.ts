...
import { CP1Data, pathsData, lineData } from './tagData';
import { styleText } from '../StyleText';
import { CPDrawer } from '../CPFunction';

export class CP1Drawer extends CPDrawer {
    ...
    public update() {
        ...
        lineGroups.attr('transform', 'translate(-269, 257) scale(-0.67, 0.67) rotate(135)');
        ...
        const filteredPaths = groups.selectAll("path")
            .style("opacity", (d) => {
                ...
                if(Array.isArray(selectedBlock) && selectedBlock.length > 1){
                    //@ts-ignore
                    if (selectedBlock[1]?.length === 0 && Array.isArray(highlightedGroup) && highlightedGroup.includes("g1")) {
                        //@ts-ignore
                ...
                if (element.type === 'path') {
                    group
                        ...
                        .style('opacity', () => {
                            ...
                            // 🔹 `highlightedGroup`이 "g1"일 경우 처리
                            //@ts-ignore
                            if ((Array.isArray(highlightedGroup) && highlightedGroup.includes("g1")) || highlightedGroup === "g1") {
                                if (Array.isArray(selectedBlock) && selectedBlock.length > 1 && Array.isArray(selectedBlock[1])) {
                                    //@ts-ignore
                                    if (selectedBlock[1].length > 1 && 
                                        (selectedBlock[1][0] === element.onClick || selectedBlock[1][1] === element.onClick)) {
                ...
                if (element.type === 'ellipse') {
                    ...
                    group
                        ...
                        .style('opacity', () => {
                            ...
                            // 🔹 highlightedGroup이 배열일 경우 "g1"이 포함되어 있는지 확인
                            if (Array.isArray(highlightedGroup)) {
                                //@ts-ignore
                                if (!highlightedGroup.includes("g1")) {
                                    return 0.3;
