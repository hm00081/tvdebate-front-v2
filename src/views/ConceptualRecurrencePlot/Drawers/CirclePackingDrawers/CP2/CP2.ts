...
export class CP2Drawer extends CPDrawer {
    private readonly topicGuideCP2GSelection: d3.Selection<SVGGElement, MouseEvent, HTMLElement, any>;
    ...
    public update() {
        ...
        // @ts-ignore
        if ((Array.isArray(highlightedGroup) && highlightedGroup.includes("g2")) ||
          highlightedGroup === "g2") {
        ...
        this.topicGuideCP2GSelection.selectAll('circle, path, ellipse, text, tspan, line').style(
            'opacity',
            function (d: any) {
                const name =
        ...
        const filteredPaths = groups.selectAll("path")
            .style("opacity", (d) => {
                ...
                //@ts-ignore
                if (Array.isArray(selectedBlock) && selectedBlock[1]?.length === 0 && Array.isArray(highlightedGroup) && highlightedGroup.includes("g2")) {
                    //@ts-ignore
                ...
                if (selectedBlock.length === 0) {
                    ...
                    if (highlightedGroup === "g2" || !highlightedGroup) {
                        return 1;
                    }
                    return 0.3;
                }
        ...
        CP2Data.forEach((groupData, i) => {
            ...
            const group = this.topicGuideCP2GSelection
                ...
                group.selectAll('*').style('opacity', () => {
                    if (Array.isArray(highlightedGroup)) {
                        //@ts-ignore
                        return highlightedGroup.includes("g2") ? 1 : 0.3;
                    }
                    return highlightedGroup === "g2" || !highlightedGroup ? 1 : 0.3;
                });       
                    ...
                    group
                        ...
                        .style('opacity', () => {
                            ...
                            if (Array.isArray(selectedBlock) && selectedBlock.length > 1) {
                                //@ts-ignore
                                if(Array.isArray(selectedBlock[1]) && selectedBlock[1].length === 0 && highlightedGroup.includes("g2")){
                            ...
                            // 🔹 `highlightedGroup`이 배열이면 `includes("g2")`로 체크
                            if (Array.isArray(highlightedGroup)) {
                                //@ts-ignore
                                if (!highlightedGroup.includes("g2")) {
                                    return 0.3;
                            ...
                            // 🔹 `highlightedGroup`이 "g2"일 경우 처리
                            //@ts-ignore
                            if ((Array.isArray(highlightedGroup) && highlightedGroup.includes("g2")) || highlightedGroup === "g2") {
                                if (Array.isArray(selectedBlock) && selectedBlock.length > 1 && Array.isArray(selectedBlock[1])) {
                                    //@ts-ignore
                                    if (selectedBlock[1].length > 1 && 
                                        (selectedBlock[1][0] === element.onClick || selectedBlock[1][1] === element.onClick)) {
                ...
                if (element.type === 'ellipse') {
                    ...
                    group
                        ...
                        .style('opacity', () => {
                            ...
                            // 🔹 highlightedGroup이 배열일 경우 "g2"이 포함되어 있는지 확인
                            if (Array.isArray(highlightedGroup)) {
                                //@ts-ignore
                                if (!highlightedGroup.includes("g2")) {
                                    return 0.3;
                ...
                if (element.type === 'text') {
                    ...
                    text
                      ...
                      .style('opacity', () => {
                        if (highlightedGroup) {
                            //@ts-ignore
                            if(element.content[0].text !== "인구절벽, 징병제에" && element.content[0].text !== "미치는 영향"
                                //@ts-ignore
                                && element.content[0].text !== "인구절벽," && element.content[0].text !== "모병제가 답인가?"
                                //@ts-ignore
