...
export class CP2Drawer extends CPDrawer {
    ...
    public update() {
        ...
        this.topicGuideCP2GSelection.selectAll('circle, path, ellipse, text, tspan, line').style('opacity', () => {
            ...
            groupData.elements.forEach((element) => {
                ...
                if (element.type === 'text') {
                    ...
                    text.on('mouseenter', (e) => {
                        // 모든 텍스트 및 tspan 요소를 완전히 보이도록 설정
                        group.selectAll('text, tspan').style('opacity', 1);
                    }).on('mouseleave', (e) => {
                        // 기본 상태로 복구
                        group
