...
export class CPDrawer {
    ...
    public constructor(protected dataStructureSet: DataStructureSet, protected transcriptViewerRef: React.RefObject<TranscriptViewerMethods>) {}
    ...
    public getTopCompoundTerms(compoundTerms: { [key: string]: number }, topN: number) {
    ...
    public handleClickText(index: number) {
        ...
        if (this.transcriptViewerRef && this.transcriptViewerRef.current) {
            ...
            this.transcriptViewerRef.current.highlightKeywords(topTerms, [], index, -1);
        }
    }

    public handleClick(index: number, event: React.MouseEvent<SVGPathElement, MouseEvent>) {
        if (!index) {
            if (index !== 0) return;
        }
    ...
    public handleMouseEnter(index: number, event: React.MouseEvent<SVGPathElement, MouseEvent>) {
        if (!index) {
            if (index !== 0) return;
        }
    ...
    public handleMouseLeave(index: number, event: React.MouseEvent<SVGPathElement, MouseEvent>) {
        if (!index) {
            if (index !== 0) return;
        }
