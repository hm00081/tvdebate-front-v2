...
export class D3Drawer {
    private readonly conceptRecurrencePlotDiv!: d3.Selection<HTMLDivElement, any, HTMLCanvasElement, any>;
    private readonly svgSelection!: d3.Selection<SVGSVGElement, MouseEvent, HTMLCanvasElement, any>;
    private readonly svgGSelection!: d3.Selection<SVGGElement, MouseEvent, HTMLCanvasElement, any>;
    ...
    private transcriptViewerRef: React.RefObject<TranscriptViewerMethods> | null = null;
    ...
    setupClickListener(transcriptViewerRef: React.RefObject<TranscriptViewerMethods>) {
        this.similarityBlocksDrawer._clickListener = (e: MouseEvent, d: SimilarityBlock) => {
            const rowIndex = d.rowUtteranceIndex;
            const colIndex = d.columnUtteranceIndex;

            const rowUtterance = this.dataStructureSet.utteranceObjectsForDrawingManager.utteranceObjectsForDrawing[rowIndex];
            const colUtterance = this.dataStructureSet.utteranceObjectsForDrawingManager.utteranceObjectsForDrawing[colIndex];

            // 키워드 빈도수 계산 함수
            const countCompoundTerms = (data: SentenceObject[]): { [key: string]: number } => {
            ...
            const getTopCompoundTerms = (compoundTerms: { [key: string]: number }, topN: number) => {
            ...
            if (transcriptViewerRef.current) {
                ...
                transcriptViewerRef.current.highlightKeywords(rowTopTerms, colTopTerms, rowIndex, colIndex);
            }
            this.similarityBlocksDrawer.setSingleBlockIndices(d.rowUtteranceIndex, d.columnUtteranceIndex);
    ...
    public constructor(
            ...
            .call(
                d3
                    ...
                    .on('zoom', (event) => {
                        ...
                        this.svgGSelection.attr('transform', `translate(${x}, ${y}) scale(${k}) rotate(${rotationAngle})`);
        ...
        this.refutationIconDrawer = new RefutationIconDrawer(dataStructureSet.utteranceObjectsForDrawingManager.utteranceObjectsForDrawing, this.svgGSelection); // 반박 아이콘 drawer
        this.refutationIconDrawerTwo = new RefutationIconDrawerTwo(dataStructureSet.utteranceObjectsForDrawingManager.utteranceObjectsForDrawing, this.svgGSelection);
        this.insistenceIconDrawer = new InsistenceIconDrawer(dataStructureSet.utteranceObjectsForDrawingManager.utteranceObjectsForDrawing, this.svgGSelection);
        this.insistenceIconDrawerTwo = new InsistenceIconDrawerTwo(dataStructureSet.utteranceObjectsForDrawingManager.utteranceObjectsForDrawing, this.svgGSelection); // 불확실 아이콘 drawer
        this.uncertainIconDrawer = new InsistenceIconDrawer(dataStructureSet.utteranceObjectsForDrawingManager.utteranceObjectsForDrawing, this.svgGSelection);
        ...
        this.similarityBlocksDrawer.mouseoverListener = (e: MouseEvent, d: SimilarityBlock) => {
            ...
            } else if (d.colUtteranceName === '김종대' || d.colUtteranceName === '장경태') {
                this.insistenceIconDrawer.similarityBlock = d;
                this.refutationIconDrawerTwo.similarityBlock = d;
            } else if (d.rowUtteranceName === '이준석' || d.rowUtteranceName === '박휘락') {
                this.refutationIconDrawerTwo.similarityBlock = d;
                this.insistenceIconDrawer.similarityBlock = d;
            } else if (d.rowUtteranceName === '김종대' || d.rowUtteranceName === '장경태') {
                this.refutationIconDrawer.similarityBlock = d;
                this.insistenceIconDrawerTwo.similarityBlock = d;
            } else {
            }
        ...
        this.participantBlocksDrawer.clickListener = (e: MouseEvent, d: UtteranceObjectForDrawing) => {
            const iconDrawers = [
        ...
        this.topicGroupsDrawer = new TopicGroupsDrawer(this.svgGSelection, debateDataSet, dataStructureSet, termType);
        this.manualSmallTGsDrawer = new TopicGroupsDrawer(this.svgGSelection, debateDataSet, dataStructureSet, termType);
        this.manualSmallTGsDrawer.color = '#000000'; // blue
        this.manualMiddleTGsDrawer = new TopicGroupsDrawer(this.svgGSelection, debateDataSet, dataStructureSet, termType);
        //this.manualMiddleTGsDrawer.color = "#ff0001";
        this.manualMiddleTGsDrawer.color = '#000001';
        this.manualBigTGsDrawer = new TopicGroupsDrawer(this.svgGSelection, debateDataSet, dataStructureSet, termType);
        // this.manualBigTGsDrawer.color = "#000000";
        this.manualBigTGsDrawer.color = '#026b02';
        this.manualPeopleTGsDrawer = new TopicGroupsDrawer(this.svgGSelection, debateDataSet, dataStructureSet, termType);
        this.manualPeopleTGsDrawer.color = '#c';
        this.lcsegEGsDrawer = new TopicGroupsDrawer(this.svgGSelection, debateDataSet, dataStructureSet, termType);
        this.CP1Drawer = new CP1Drawer(this.svgGSelection, dataStructureSet, transcriptViewerRefs);
        this.CP2Drawer = new CP2Drawer(this.svgGSelection, dataStructureSet, transcriptViewerRefs);
        this.CP3Drawer = new CP3Drawer(this.svgGSelection, dataStructureSet, transcriptViewerRefs);
        this.CP4Drawer = new CP4Drawer(this.svgGSelection, dataStructureSet, transcriptViewerRefs);
        this.CP5Drawer = new CP5Drawer(this.svgGSelection, dataStructureSet, transcriptViewerRefs);
        this.CP6Drawer = new CP6Drawer(this.svgGSelection, dataStructureSet, transcriptViewerRefs);
        this.CP7Drawer = new CP7Drawer(this.svgGSelection, dataStructureSet, transcriptViewerRefs);
        this.PlotChartDrawer = new PlotChartDrawer(this.svgGSelection, dataStructureSet, transcriptViewerRefs, this);
        this.lcsegEGsDrawer.color = '#cc9900'; // yellow color
        this.svgSelection.on('click', (event) => {
            _.forEach(dataStructureSet.similarityBlockManager.similarityBlocks, (similarityBlock) => {
    ...
    public centerConceptualRecurrentPlot() {
        const utteranceObjectsForDrawing = this.dataStructureSet.utteranceObjectsForDrawingManager.utteranceObjectsForDrawing;
        const ar = [];
        const arr = [];
        ...
        if (utteranceObjectsForDrawing.length !== 0) {
            const lastUtteranceObjectForDrawing = utteranceObjectsForDrawing[utteranceObjectsForDrawing.length - 1];

            const minusWidth = lastUtteranceObjectForDrawing.beginningPointOfXY + lastUtteranceObjectForDrawing.width;
            ...
            this.svgGSelection.attr('transform', `translate(${adjustedWidth}, ${adjustedHeight})`);
