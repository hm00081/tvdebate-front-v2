...
export class SimilarityBlocksDrawer {
    ...
    private safeGroupMatch(groupKey: string | null | undefined, rowIdx: number, colIdx: number): boolean {
    ...
    const range = groupKey ? groupRanges[groupKey] : undefined;
    return !!(range &&
...
public update() {
    const self = this;
    const { highlightedGroup } = store.getState().highlight as {
    ...
    const merged = enter.merge(similarityRectGSelectionDataBound);
    ...
    merged
        ...
        .style("fill", d => {
            ...
            return isWithinRange
                ? self.fillColorOfSimilarityBlock(
                      d,
                      self.utteranceObjectsForDrawing,
                      self.similarityBlockGroup,
                      self.participantDict,
                      self._standardHighPointOfSimilarityScore,
                      self._coloringSelfSimilarities,
                      self._coloringRebuttal
                  )
                : "none";
        })
        .style("cursor", d => {
        ...
        .style("opacity", function () {
            ...
            if (selectedBlock && selectedBlock.length > 1 && Array.isArray(selectedBlock[1])) {
                ...
                if (selectedBlock[1].length === 0) {
                    ...
                    if (highlightedNames.includes(rowName) && highlightedNames.includes(colName)) {
                        const group0 = highlightedGroup?.[0];
                        const group1 = highlightedGroup?.[1];
                        if (self.safeGroupMatch(group0, rowIdx, colIdx) && self.safeGroupMatch(group1, rowIdx, colIdx)) {
                            return maxOpacity;
                        }
                    }
                    return 0.05;
                ...
                if (highlightedClasses.length === 1) {
                    if (selectedBlock[1][0] === rowIdx || selectedBlock[1][1] === colIdx) return 1;
                }
            ...
            if (highlightedClasses.length > 0) {
                ...
                const highlightedNames = highlightedClasses.map(cls => participants[cls]).filter(Boolean);
                if (highlightedClasses.length === 1) {
                    const isHighlighted = highlightedNames.includes(rowName) || highlightedNames.includes(colName);
                    return isHighlighted ? maxOpacity : 0.05;
                } else if (highlightedClasses.length <= 4) {
                    if (highlightedNames.includes(rowName) && highlightedNames.includes(colName)) {
                        return maxOpacity;
                    } else {
                        return 0.05;
            ...
            if (Array.isArray(highlightedGroup)) {
                const matched = highlightedGroup.some(group =>
                    self.safeGroupMatch(group, rowIdx, colIdx)
                );
                if (matched) return 1;
            }

            if (typeof highlightedGroup === "string" &&
                self.safeGroupMatch(highlightedGroup, rowIdx, colIdx)) {
                return 1;
            }
        ...
        .style("stroke", d => {
            ...
            return self._showEngagementPoint && d.engagementPoint ? "rgb(97, 64, 65)" : null;
        })
        .on("click", function (event: MouseEvent, d: SimilarityBlock) {
            ...
            if (disallowedPairs.some(([a, b]) => a === rowName && b === colName)) return;
            ...
            if (!allowedPairs.some(([a, b]) => a === rowName && b === colName)) return;

            self._selectedBlockIndices.push([d.rowUtteranceIndex, d.columnUtteranceIndex]);
            self.updateSelectedBlock();
            event.stopPropagation();

            if (self._clickListener) {
                self._clickListener(event, d);
            }
        });

    enter.append("title").text((d) => {
        const argumentScore = self.calculateArgumentScore(d);
        return `findArgument: ${d.refutation || 'none'},
...
argumentScore: ${argumentScore}`;
    ...
    // no error
    private groupRanges: Record<string, { row: [number, number]; col: [number, number] }> = {
        g1: { row: [0, 18], col: [0, 19] },
        g2: { row: [14, 37], col: [15, 38] },
        g3: { row: [23, 58], col: [24, 59] },
        g4: { row: [42, 79], col: [43, 80] },
    ...
    private getMatchingGroupIds(rowIdx: number, colIdx: number): string[] {
        return Object.entries(this.groupRanges)
            .filter(([_, range]) =>
                rowIdx >= range.row[0] &&
                rowIdx <= range.row[1] &&
                colIdx >= range.col[0] &&
        ...
        this.conceptSimilarityRectGSelection
            ...
            .each((d) => {
                const isSelected = this._selectedBlockIndices.some(
                    ([row, col]) => row === d.rowUtteranceIndex && col === d.columnUtteranceIndex
                );
    
                if (!isSelected) return;
    
                const groupIds = this.getMatchingGroupIds(d.rowUtteranceIndex, d.columnUtteranceIndex);
                const p1 = this.participants[d.rowUtteranceName];
                const p2 = this.participants[d.colUtteranceName];
    
                // 상태 store 업데이트
                store.dispatch(setSelectedBlock([[p1, p2], [d.rowUtteranceIndex, d.columnUtteranceIndex]]));
                store.dispatch(setHighlightedGroup(
                    groupIds.length === 1 ? [groupIds[0], groupIds[0]] : [groupIds[0], groupIds[1]]
                ));
            ...
            .style("stroke", (d) =>
                this._selectedBlockIndices.some(
                    ([row, col]) => row === d.rowUtteranceIndex && col === d.columnUtteranceIndex
                ) ? "#fc2c34" : null
            )
            .style("fill", (d) =>
