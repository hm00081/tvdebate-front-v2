...
export default class Pk {
    ...
    public makePkBasedOnSentence(sentenceIndexesForSegmentsOfModel: number[]) {
        ...
        for (let i = 0; i < this.totalSentenceObjects.length - this.k; i++) {
            const deltaOfHuman = this.makeDelta(i, this.k, this.sentenceIndexesForSegmentsOfPeople);
            const deltaOfModel = this.makeDelta(i, this.k, sentenceIndexesForSegmentsOfModel);
    ...
    private makeDelta(i: number, k: number, sentenceIndexesForSegments: number[]): 1 | 0 {
        // [1, 3, 5, 7, 9]
        // find (i)'s order in manualSentenceIndexesOfSegments
        const index1 = _.find(sentenceIndexesForSegments, (sentenceIndex) => sentenceIndex >= i);
        // find (i+k)'s order in manualSentenceIndexesOfSegments
        const index2 = _.find(sentenceIndexesForSegments, (sentenceIndex) => sentenceIndex > i + k);
